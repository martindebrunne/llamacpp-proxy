 ...
 * @param {string} incomingModel - The model name from the request
 * @returns {string} The real model name without suffix
 */
function extractRealModel(incomingModel) {
...
// Helper to check if model has a suffix
function hasModelSuffix(model) {
  return model && (model.includes("-Think") || model.includes("-No-Think"));
}
...
function createSseChunkFromTemplate(baseChunk, choice, delta, model) {
  return {
        ...
        for (const choice of chunk.choices) {
          ...
          if (delta && typeof delta === "object") {
            if (delta.role === "assistant" && !assistantRoleSent) {
              res.write(
                serializeSseEvent(
                  createSseChunkFromTemplate(chunk, choice, { role: "assistant" }, mapped?.model)
                )
              );
            ...
            if (hasUsableContent(delta.content)) {
              res.write(
                serializeSseEvent(
                  createSseChunkFromTemplate(chunk, choice, { content: delta.content }, mapped?.model)
                )
              );
            ...
            if (isNonEmptyArray(delta.tool_calls)) {
              res.write(
                serializeSseEvent(
                  createSseChunkFromTemplate(chunk, choice, { tool_calls: delta.tool_calls }, mapped?.model)
                )
              );
              sawToolCalls = true;
            }
            ...
            if (hasUsableContent(message.content)) {
              res.write(
                serializeSseEvent(
                  createSseChunkFromTemplate(chunk, choice, { content: message.content }, mapped?.model)
                )
              );
              sawUsefulContent = true;
            }
            ...
            if (isNonEmptyArray(message.tool_calls)) {
              res.write(
                serializeSseEvent(
                  createSseChunkFromTemplate(chunk, choice, { tool_calls: message.tool_calls }, mapped?.model)
                )
              );
              sawToolCalls = true;
            }
...
// Les modèles sont dynamiques basés sur ce qui est disponible en upstream
...
app.listen(PROXY_PORT, PROXY_HOST, () => {
  ...
  info("Dynamic model detection enabled - real model extracted from incoming requests");
});
