...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      let upstreamModels: { id: string; object?: string; owned_by?: string }[] = [];

      if (upstreamResponse.ok) {
        const upstreamData = await upstreamResponse.json() as { data?: unknown[] };
        upstreamModels = (upstreamData.data || []) as { id: string; object?: string; owned_by?: string }[];
      }
      ...
      const modelData = upstreamModels.map((model) => {
        ...
        return [
          { id: `${modelId}-Think`, object: "model" as const, owned_by: "local" },
          { id: `${modelId}-No-Think`, object: "model" as const, owned_by: "local" },
        ];
      }).flat();

      // Add upstream models without variants
      modelData.push(...upstreamModels.map((m) => ({ id: m.id, object: "model" as const, owned_by: m.owned_by || "local" })));
